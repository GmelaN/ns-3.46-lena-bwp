...
static double g_bwp0Freq = 0.70e9;
static double g_bwp1Freq = 3.50e9;
            ...
            const std::string& runLabel)
{
    ...
    std::cout << "errorModel=" << g_errorModel
              << " bwp0=" << std::fixed << std::setprecision(3) << (g_bwp0Freq / 1e9) << "GHz"
              << " bwp1=" << std::fixed << std::setprecision(3) << (g_bwp1Freq / 1e9) << "GHz"
    ...
    CcBwpCreator::SimpleOperationBandConf bwp0Conf(g_bwp0Freq, g_bwp0BwHz, 1);
    CcBwpCreator::SimpleOperationBandConf bwp1Conf(g_bwp1Freq, g_bwp1BwHz, 1);
    ...
    for (uint32_t i = 0; i < g_numUes; ++i)
    {
        ...
        double bwpFreq = (bwpId == 0) ? g_bwp0Freq : g_bwp1Freq;
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("bwp0Freq", "Center frequency for BWP 0 (Hz)", g_bwp0Freq);
    cmd.AddValue("bwp1Freq", "Center frequency for BWP 1 (Hz)", g_bwp1Freq);
