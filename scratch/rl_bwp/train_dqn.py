...
def main():
    parser = argparse.ArgumentParser(description="Train quantile DQN over OpenGym/ns3 env.")
    parser.add_argument("--backend", choices=["mock", "ns3"], default="mock")
    parser.add_argument("--run-name", type=str, default="dqn_bwp_mcs_ue1")
    ...
    out_dir = os.path.join("/home/jshyeon/ns-3.46-bwp/scratch/rl_bwp/runs", args.run_name, "dqn")
    ...
    online_net = DqnNet(obs_dim, args.hidden_dim, action_dim, args.num_quantiles).to(device)
    target_net = DqnNet(obs_dim, args.hidden_dim, action_dim, args.num_quantiles).to(device)
