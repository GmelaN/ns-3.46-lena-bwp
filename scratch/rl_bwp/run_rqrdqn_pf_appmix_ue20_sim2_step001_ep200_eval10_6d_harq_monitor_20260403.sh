...
ROOT_REL="${1:-rqrdqn_pf_appmix_ue20_sim2_eval5x10_ep200_step001_6d_harq_monitor_20260403}"
    ...
    for key, stats in means.items():
        for metric, pattern in metric_labels.items():
            if re.match(pattern, key):
                ...
                all_rows.append({"seed": seed, "ue": ue, "metric": metric, "value": value})
                metric_to_values[metric].append(value)
                break
    for metric, values in metric_to_values.items():
        if not values:
            continue
        seed_rows.append({"seed": seed, "metric": metric, "mean": sum(values) / len(values), "reward_mean": reward_mean})
