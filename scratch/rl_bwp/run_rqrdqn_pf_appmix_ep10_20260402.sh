...
ROOT_REL="${1:-rqrdqn_pf_appmix_ue20_sim2_eval5_ep10_step001_opt_20260402}"
...
./ns3gym-venv/bin/python3 scratch/rl_bwp/train_rqrdqn.py \
  ...
  >"$RUN_ROOT/logs/rqrdqn_pf_train.log" 2>&1

./ns3gym-venv/bin/python3 scratch/rl_bwp/eval_rqrdqn.py \
  --backend ns3 \
  --model-path "$RUN_ROOT/rqrdqn_pf_train/final_model.pt" \
  --seed 1 \
  ...
  --ns3-script "$NS3_SCRIPT" \
  --port "$((PORT_BASE + 2))" \
  --output-json "$RUN_ROOT/eval/rqrdqn_pf_eval.json" \
  --step-log-csv "$RUN_ROOT/eval/rqrdqn_pf_eval_steps.csv" \
  --ns3-arg trafficModel=mixed \
  --ns3-arg schedulerPolicy=pf \
  >"$RUN_ROOT/logs/rqrdqn_pf_eval.log" 2>&1

python3 - <<'PY' "$RUN_ROOT/eval/rqrdqn_pf_eval.json" "$RUN_ROOT/eval/eval_percentiles.csv"
import csv
import json
import sys
