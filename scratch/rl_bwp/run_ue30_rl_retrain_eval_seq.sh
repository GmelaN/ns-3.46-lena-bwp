...
RUN_ROOT="scratch/rl_bwp/runs/ue30_retrain_20260326"
...
run_train_drqn() {
  ...
  if pgrep -af "train_drqn.py .*--run-name $run_name" >/dev/null; then
    echo "[wait] train running: $run_name"
    while pgrep -af "train_drqn.py .*--run-name $run_name" >/dev/null; do
      sleep 15
    done
    return
  fi
...
run_train_rqrdqn() {
  ...
  if pgrep -af "train_rqrdqn.py .*--run-name $run_name" >/dev/null; then
    echo "[wait] train running: $run_name"
    while pgrep -af "train_rqrdqn.py .*--run-name $run_name" >/dev/null; do
      sleep 15
    done
    return
  fi
...
run_scenario() {
  ...
  run_train_drqn "ue30_retrain_20260326/$scenario/drqn_pf_train" "$((port_base + 1))" \
    "${common_args[@]}" \
  ...
  run_train_rqrdqn "ue30_retrain_20260326/$scenario/rqrdqn_bwponly_train" "$((port_base + 3))" \
    --drqn-profile \
    "${common_args[@]}" \
    --ns3-arg schedulerPolicy=pf \
  ...
  run_train_rqrdqn "ue30_retrain_20260326/$scenario/rqrdqn_full_train" "$((port_base + 5))" \
    "${common_args[@]}" \
    --ns3-arg schedulerPolicy=pf \
  ...
  run_train_drqn "ue30_retrain_20260326/$scenario/drqn_aeq_train" "$((port_base + 7))" \
    "${common_args[@]}" \
...
run_scenario mixed_mobile mixed 8 500 true 63030
run_scenario legacy_bursty_heavy legacy 160 1000 false 63130

echo "[done] ue30 RL retrain/eval sequence finished"
