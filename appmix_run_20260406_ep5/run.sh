...
launch_rqrdqn_pf() {
  ...
  (
    "${PYTHON}" "${ROOT_DIR}/scratch/rl_bwp/train_rqrdqn.py" \
      --backend ns3 \
      --run-name "appmix_rqrdqn_pf_ue20_sim5_step010_ep5_harq" \
      --seed 1001 \
      --num-ues "${NUM_UES}" \
...
launch_drqn_aequitas() {
  ...
  (
    "${PYTHON}" "${ROOT_DIR}/scratch/rl_bwp/train_drqn.py" \
      --backend ns3 \
      --run-name "appmix_drqn_aequitas_ue20_sim5_step010_ep5_harq" \
      --seed 2001 \
      --num-ues "${NUM_UES}" \
      --episode-time-s "${SIM_TIME}" \
      --step-time-s "${STEP}" \
    ...
    train_pid=$!
    sleep 2
    MONITOR_TAG="drqn_aequitas" \
      "${ROOT_DIR}/scratch/rl_bwp/monitor_ns3_reconnector.sh" "${train_pid}" "${PORT_DRQN_AEQ}" 10 5 \
      "${NS3_SCRIPT}" --numUes="${NUM_UES}" --simTime="${SIM_TIME}" --envStepTime="${STEP}" \
      --enableOpenGym=true --trafficModel=mixed --schedulerPolicy=aequitas \
      --aequitasEnableMcsSelection=true --enableHarqReTx=true &
    monitor_pid=$!
    wait "${train_pid}"
    kill "${monitor_pid}" 2>/dev/null || true
  ) &
...
launch_drqn_pf() {
  ...
  (
    "${PYTHON}" "${ROOT_DIR}/scratch/rl_bwp/train_drqn.py" \
      --backend ns3 \
      --run-name "appmix_drqn_pf_ue20_sim5_step010_ep5_harq" \
      --seed 3001 \
      --num-ues "${NUM_UES}" \
      --episode-time-s "${SIM_TIME}" \
      --step-time-s "${STEP}" \
    ...
    sleep 2
    MONITOR_TAG="drqn_pf" \
      "${ROOT_DIR}/scratch/rl_bwp/monitor_ns3_reconnector.sh" "${train_pid}" "${PORT_DRQN_PF}" 10 5 \
      "${NS3_SCRIPT}" --numUes="${NUM_UES}" --simTime="${SIM_TIME}" --envStepTime="${STEP}" \
      --enableOpenGym=true --trafficModel=mixed --schedulerPolicy=pf \
      --enableHarqReTx=true --aequitasEnableMcsSelection=false &
    monitor_pid=$!
...
postprocess_reward_bins() {
  local run_dir="$1"
  local src="${run_dir}/train_reward_bins_300.csv"
  local dst="${run_dir}/train_reward_bins_200.csv"
  if [[ -f "${src}" ]]; then
    cp -f "${src}" "${dst}"
  fi
}
...
main() {
  ...
  postprocess_reward_bins "${ROOT_DIR}/scratch/rl_bwp/runs/appmix_rqrdqn_pf_ue20_sim5_step010_ep5_harq"
  postprocess_reward_bins "${ROOT_DIR}/scratch/rl_bwp/runs/appmix_drqn_aequitas_ue20_sim5_step010_ep5_harq"
  postprocess_reward_bins "${ROOT_DIR}/scratch/rl_bwp/runs/appmix_drqn_pf_ue20_sim5_step010_ep5_harq"
}
